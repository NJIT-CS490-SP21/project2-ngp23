...
def score_board():
    ...
    score_dict = []
    score_people = models.Person.query.order_by(models.Person.score.desc()).all()
...
def on_login(data): # data is whatever arg you pass in your emit call on client
    ...
    add_db(data)
    if "X" not in user_names:
        user_names["X"] = data["setUser"]
    elif "O" not in user_names:
        user_names["O"] = data["setUser"]
    else:
        user_list.append(data["setUser"])
        user_names["spec"] = user_list
    print(user_names)
    score = score_board()
    socketio.emit('resetStats', score, broadcast=True, include_self=False)
    socketio.emit('login', user_names, broadcast=True, include_self=False)
